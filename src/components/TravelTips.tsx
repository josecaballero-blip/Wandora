...
export default function TravelTips() {
  ...
  return (
    <section id="tips" className="py-20 lg:py-28 bg-gray-900 text-white">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {tips.map((tip, i) => (
            <motion.div
              ...
              className={`group relative p-6 sm:p-8 rounded-2xl sm:rounded-3xl border transition-all duration-500 hover:bg-gray-800/80 ${
                theme === "dark"
                  ? "bg-gray-800/50 border-gray-700/50 hover:border-teal-500/30"
                  : "bg-gray-800/50 border-gray-700/50 hover:border-teal-500/30"
              }`}
            >
              <div
                className={`text-5xl sm:text-6xl font-black bg-gradient-to-br ${tip.color} bg-clip-text text-transparent opacity-20 group-hover:opacity-40 transition-opacity duration-500`}
              >
                {tip.number}
              </div>
